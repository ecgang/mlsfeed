...
if(empty($post_id) && !advmls_is_tax()) {
    return;
}

if(is_page_template(array('template/template-search.php')) && advmls_option('search_result_page') == 'half_map') {
    return;
}

if(advmls_is_tax()) {
    if(advmls_option('tax_show_map')) {
...
// Header Search Start 
if( advmls_search_needed() ) {

    $search_enable = advmls_option('main-search-enable');
    $search_position = advmls_option('search_position');
    $search_pages = advmls_option('search_pages');
    $search_selected_pages = advmls_option('header_search_selected_pages');
