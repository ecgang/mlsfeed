...
$documents_download = advmls_option('documents_download');

$prop_id = advmls_get_listing_data('property_id');
$prop_price = advmls_get_listing_data('property_price');
...
$property_status = advmls_taxonomy_simple('property_status');
$property_type = advmls_taxonomy_simple('property_type');
$garage_size = advmls_get_listing_data('property_garage_size');
$additional_features_enable = advmls_get_listing_data('additional_features_enable');
$additional_features = get_post_meta( get_the_ID(), 'additional_features', true);

$icon_prop_id = advmls_option('icon_prop_id', false, 'url' );
$icon_bedrooms = advmls_option('icon_bedrooms', false, 'url' );
$icon_rooms = advmls_option('icon_rooms', false, 'url' );
$icon_bathrooms = advmls_option('icon_bathrooms', false, 'url' );
$icon_prop_size = advmls_option('icon_prop_size', false, 'url' );
$icon_prop_land = advmls_option('icon_prop_land', false, 'url' );
$icon_garage_size = advmls_option('icon_garage_size', false, 'url' );
$icon_garage = advmls_option('icon_garage', false, 'url' );
$icon_year = advmls_option('icon_year', false, 'url' );

$bathrooms_text = ($bathrooms > 1 ) ? advmls_option('spl_bathrooms', 'Bathrooms') : advmls_option('spl_bathroom', 'Bathroom');

$bedrooms_text = ($bedrooms > 1 ) ? advmls_option('spl_bedrooms', 'Bedrooms') : advmls_option('spl_bedroom', 'Bedroom');

$rooms_text = ($rooms > 1 ) ? advmls_option('spl_rooms', 'Rooms') : advmls_option('spl_room', 'Room');

$garage_label = ($garage > 1 ) ? advmls_option('spl_garages', 'Garages') : advmls_option('spl_garage', 'Garage');


$hide_fields = advmls_option('hide_detail_prop_fields');
?>
<div class="fw-property-description-wrap fw-property-section-wrap" id="property-description-wrap">
	<div class="block-wrap">
		<div class="block-title-wrap">
			<h2><?php echo advmls_option('sps_description', 'Description'); ?></h2>	
		...
		<div class="block-content-wrap">
			...
			<div class="fw-property-documents-wrap">
				<div class="block-title-wrap">
					<h3>
						<span><?php echo advmls_option('sps_documents', 'Property Documents'); ?></span>
					</h3>
				</div><!-- block-title-wrap -->
				<div class="property-documents">
					<?php 
					foreach( $attachments as $attachment_id ):
						$attachment_meta = advmls_get_attachment_metadata($attachment_id); 

						if(!empty($attachment_meta )):
						?>
						<div class="property-document-title">
							<i class="advmls-icon icon-task-list-plain-1 mr-1"></i> <?php echo esc_attr( $attachment_meta->post_title ); ?> - 
							<?php if( $documents_download == 1 ) {
			                    if( is_user_logged_in() ) { ?>
			                    <a href="<?php echo esc_url( $attachment_meta->guid ); ?>" target="_blank"><?php esc_html_e( 'Download', 'advmls' ); ?></a>
			                    <?php } else { ?>
			                        <a href="#" data-toggle="modal" data-target="#login-register-form"><?php esc_html_e( 'Download', 'advmls' ); ?></a>
			                    <?php } ?>
			                <?php } else { ?>
			                    <a href="<?php echo esc_url( $attachment_meta->guid ); ?>" target="_blank"><?php esc_html_e( 'Download', 'advmls' ); ?></a>
			                <?php } ?>
			...
			<div class="fw-property-details-wrap">
				<div class="block-title-wrap">
					<h3>
						<span><?php echo advmls_option('sps_details', 'Details'); ?></span>
					</h3>
				</div><!-- block-title-wrap -->
				<div class="d-flex justify-content-center fw-property-amenities-wrap clearfix">
					...
					<div class="fw-property-amenities">
						<div class="d-flex align-items-center">
							...
							<div class="fw-property-amenities-data">
								<?php echo advmls_option('spl_prop_id', 'Property ID'); ?><br>
								<?php echo advmls_propperty_id_prefix( $prop_id ); ?>
							</div>
						</div>
					</div>
					<?php } ?>
        			...
        			<div class="fw-property-amenities">
						<div class="d-flex align-items-center">
							...
							<?php } ?>
							<div class="fw-property-amenities-data">
								<?php echo advmls_option('spl_prop_size', 'Property Size'); ?><br>
								<?php echo advmls_property_size( 'after' ); ?>
							</div>
						</div>
					</div>
					<?php } ?>
					...
					<div class="fw-property-amenities">
						<div class="d-flex align-items-center">
							...
							<?php } ?>
							<div class="fw-property-amenities-data">
								<?php echo advmls_option('spl_land', 'Land Area'); ?><br>
								<?php echo advmls_property_land_area( 'after' ); ?>
							</div>
						</div>
					</div>
					<?php } ?>
			        ...
			        <div class="fw-property-amenities">
				        <div class="d-flex align-items-center">
							...
							<div class="fw-property-amenities-data">
								<?php echo advmls_option('spl_garage_size', 'Garage Size'); ?><br>
			        ...
			        <div class="fw-property-amenities">
						<div class="d-flex align-items-center">
							...
							<?php } ?>
							<div class="fw-property-amenities-data">
								<?php echo advmls_option('spl_year_built', 'Year Built'); ?><br>
					...
					<?php
					//Custom Fields
			        if(class_exists('advmls_Fields_Builder')) {
			        $fields_array = advmls_Fields_Builder::get_form_fields(); 
			            ...
			            if(!empty($fields_array)) {
			                foreach ( $fields_array as $value ) {
			                    ...
			                    $field_title = advmls_wpml_translate_single_string($field_title);

			                    if( $meta_type == true ) {
			                        $data_value = advmls_wpml_translate_single_string($data_value);
			                    } else {
			                        $data_value = advmls_array_to_comma($data_value);
			                    }

			                    $field_id = advmls_clean_20($value->field_id);

			                    if(!empty($data_value) && $hide_fields[$field_id] != 1) {

			                    	$custom_icon = advmls_option('c_'.$field_id, false, 'url' );
			...
			<span class="small-text grey"><i class="advmls-icon icon-calendar-3 mr-1"></i> <?php esc_html_e( 'Updated on', 'advmls' ); ?> <?php the_modified_time('F j, Y'); ?> <?php esc_html_e( 'at', 'advmls' ); ?> <?php the_modified_time('g:i a'); ?> </span>	
			<?php } ?>
