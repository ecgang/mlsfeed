...
foreach ($agent_listing_ids as $listing_id) {
	$term = get_the_terms( $listing_id, 'property_city' );
...
foreach ($taxnonomies as $slug => $name) {
	$count = advmls_realtor_stats('property_city', 'fave_agents', get_the_ID(), $slug);
	if(!empty($count)) {
		$total_count[] = $count;
...
<div class="agent-profile-chart-wrap">
	<h2><?php echo wp_kses(__( '<span>Property</span> Cities', 'advmls' ), advmls_allowed_html() ); ?></h2>
				...
				if(!empty($taxs_list_data) && !empty($total_count)) {
					foreach ($taxs_list_data as $taxnonomy) { $j++;
						...
						if($j <= $total_records) {
							...
							echo '<li class="stats-data-'.$j.'">
									<i class="advmls-icon icon-sign-badge-circle mr-1"></i> <strong>'.esc_attr($percent).'%</strong> <span>'.esc_attr($taxnonomy).'</span>
								</li>';
							}
						}
						$k++;
					...
					if(!empty($others)) {
						...
						echo '<li class="stats-data-'.$num.'">
								<i class="advmls-icon icon-sign-badge-circle mr-1"></i> <strong>'.round($other_percent).'%</strong> <span>'.esc_html__('Other', 'advmls').'</span>
							</li>';
					}
