...
$features_limit = advmls_option('features_limit');
...
if( taxonomy_exists('property_feature') ) {
    ...
    if ( !empty($prop_features)  && ! is_wp_error( $prop_features )  ) {
        ...
        foreach ($prop_features as $feature):
            ...
            echo '<input class="'.advmls_get_ajax_search().'" name="feature[]" type="checkbox" '.$checked_feature.' value="' . esc_attr( $feature->slug ) . '">';
