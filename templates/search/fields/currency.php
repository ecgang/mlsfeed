<?php 
$multi_currency = get_option('multi_currency');
if($multi_currency == 1 ) {
    if(class_exists('advmls_Currencies')) {

        $searched_currency = isset($_GET['currency']) ? $_GET['currency'] : '';

        $currencies = advmls_Currencies::get_currency_codes();
        if($currencies) {
            ...
            echo '<option value="">'.get_option('srh_currency', 'Currency').'</option>';
            echo '<option value="">'.get_option('srh_any', 'Any').'</option>';
