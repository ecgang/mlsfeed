<?php
$search_builder = advmls_search_builder();
...
if( advmls_is_half_map() ) {
	...
	$ajax_url_update = 'advmls-search-filters-js';
}

unset($search_fields['placebo']);
unset($search_fields['price']);
$multi_currency = advmls_option('multi_currency');
...
if(advmls_option('price_range_mobile')) {
...
<section id="overlay-search-advanced-module" class="overlay-search-advanced-module <?php echo esc_attr($search_class); ?>">
	<div class="search-title">
		<?php esc_html_e('Search', 'advmls'); ?>
		<button type="button" class="btn overlay-search-module-close"><i class="advmls-icon icon-close"></i></button>
	</div>
	<form class="advmls-search-form-js <?php echo esc_attr($ajax_url_update); ?>" method="get" autocomplete="off" action="<?php echo esc_url( advmls_get_search_template_link() ); ?>">
	...
	<div class="row">
		...
		<?php if(advmls_option('search_other_features_mobile')) { ?>
		<div class="col-12">
